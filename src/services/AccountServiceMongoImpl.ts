...
class AccountServiceMongoImpl implements AccountService {
    ...
    async setRole(_id: string, newRole: string): Promise<Employee> {
        const emp = await this.getEmployeeById(_id)
        const role = checkRole(newRole);
        const updated = await EmployeeModel.findOneAndUpdate({_id}, {
            $set: {roles: newRole}
        }, {new: true}).exec();
